...
w <- gbasicdialog(title = "Are you online", do.buttons = T, handler = function(h, ...) 
  online <<- svalue(selection))
selection <- gcombobox(c("Online", "Offline"), editable = F, expand = T, container = w)
...
if(df_donor[df_donor$status %in% j & df_donor$appeal %in% i, ][1, 4] == 0) {
