...
z <- xtable::xtable(prettyNum(df_total, ' '), caption = 'Overview of amounts donated/firmly pledged as a percentage of total requested, by appeal', digits = 0, label = "tab_overview")
...
for (j in list_status) {
  ...
  for (i in df_filter$appeal[df_filter$status %in% j]) {
    ...
    z <- xtable::xtable(prettyNum(df_selected, ' '), caption = paste0('Overview of amounts donated/firmly pledged as a percentage of total requested: ', i), digits = 0, label = paste0('tab_', i))
    ...
    if(df_donor[df_donor$status %in% j & df_donor$appeal %in% i, ][1, 4] %in% 0) {
      ...
      comment$command  <- c(paste0("\\hline \n\\emph{\\footnotesize{Amount requested: US\\$ ", prettyNum(df_donor[df_donor$status %in% j & df_donor$appeal %in% i, ][1, 4], ' '),"}}\n"))
    }
    ...
    if(df_donor[df_donor$status %in% j & df_donor$appeal %in% i, ][1, 4] %in% 0) {
      ...
      cat(paste0("\n\\marginnote{Amount requested: US\\$ ", prettyNum(df_donor[df_donor$status == j & df_donor$appeal == i, ][1, 4], ' '),"}\n"))
    }
