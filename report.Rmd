...
xtable::print.xtable(z, type = 'latex', booktabs = T, caption.placement = "top", add.to.row = list(pos = as.list(rws), command = col), include.rownames=F, comment=F, tabular.environment ='longtable', floating = FALSE)
```
\end{fullwidth}

```{r, fig_pop, fig.cap='Overview of amounts donated/firmly pledged as a percentage of total requested, by appeal', fig.pos='!htbp', fig.height=4, fig.fullwidth = F}
...
PlotBar <- function (data, xvar = "key", yvar = "value", colscheme = "Blues",  yaxis = "", legend = "") {
  ...
  # tmp[['Appeal']] <- tmp[['Appeal']] %>% factor(levels = tmp[['Appeal']] %>% rev(.))
