...
z <- xtable::xtable(prettyNum(df_total, big.mark = ' '), caption = 'Overview of amounts donated/firmly pledged as a percentage of total requested, by appeal', digits = 0, label = "tab_overview")
...
PlotBarHorizontal <- function (data, xvar = "key", yvar = "value", colscheme = "Blues",  yaxis = "", legend = "") {
...
PlotBarHorizontal(data = data_filtered()[[1]], xvar = 'appeal', yvar = 'Funded (%)', yaxis = 'Funded (%)' 
...
for (j in list_status) {
  ...
  for (i in df_filter$appeal[df_filter$status %in% j]) {
    ...
    z <- xtable::xtable(prettyNum(df_selected, big.mark = ' '), caption = paste0('Overview of amounts donated/firmly pledged as a percentage of total requested: ', i), digits = 0, label = paste0('tab_', i))
    ...
    if(df_donor[df_donor$status %in% j & df_donor$appeal %in% i, ][1, 4] %in% 0) {
      ...
      comment$command  <- c(paste0("\\hline \n\\emph{\\footnotesize{Amount requested: US\\$ ", prettyNum(df_donor[df_donor$status %in% j & df_donor$appeal %in% i, ][1, 4], big.mark = ' '),"}}\n"))
    }
    ...
    if(df_donor[df_donor$status %in% j & df_donor$appeal %in% i, ][1, 4] %in% 0) {
      ...
      cat(paste0("\n\\marginnote{Amount requested: US\\$ ", prettyNum(df_donor[df_donor$status == j & df_donor$appeal == i, ][1, 4], big.mark = ' '),"}\n"))
    }
