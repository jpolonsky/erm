...
# read script lines from website and evaluate
script <- getURL("https://raw.githubusercontent.com/jpolonsky/erm_finan_rep/master/my_functions.R", ssl.verifypeer = FALSE)
eval(parse(text = script), envir = .GlobalEnv)
@

<<cache=FALSE, message=FALSE, echo=FALSE, warning=FALSE>>=
script <- getURL("https://raw.githubusercontent.com/jpolonsky/erm_finan_rep/master/L3_emergencies_2015.R", ssl.verifypeer = FALSE)
eval(parse(text = script), envir = .GlobalEnv)
@

<<cache=FALSE, message=FALSE, echo=FALSE, warning=FALSE>>=
script <- getURL("https://raw.githubusercontent.com/jpolonsky/erm_finan_rep/master/test.R", ssl.verifypeer = FALSE)
eval(parse(text = script), envir = .GlobalEnv)
@
...
if(nrow(df_selected)>1) {
      rws <- seq(1, nrow(df_selected) - 1, by = 2)
    }
