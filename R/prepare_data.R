...
filename <- fileChoose()
listSheets <- readxl::excel_sheets(filename)
...
df_raw <- readxl::read_excel(filename, sheet = listSheets[grep("Contribution", listSheets)], skip = 1)
df_extra <- readxl::read_excel(filename, sheet = listSheets[grep(paste0("ctrbns ", year), listSheets)], skip = 1)
df_filter <- readxl::read_excel(filename, sheet = listSheets[grep(paste0("SRP ", year), listSheets)], skip = 2)
